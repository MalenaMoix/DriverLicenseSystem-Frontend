...
const IssueLicenseContainer = ({ history }) => {
		...
		if (licenseOwner.document === '') {
			...
			try {
				const res = await fetch(
					`http://localhost:9090/owner/${licenseOwner.document}`
				);
				...
				toaster.danger('ERROR AL BUSCAR TITULAR EN LA BASE DE DATOS.', {
					duration: 5,
				});
			}
		}
	...
	const getCostAndValidUntil = async (e) => {
		e.preventDefault();
		if (ownerFound) {
			const { observations } = license;
			try {
				const res = await fetch(
					`http://localhost:9090/license/${license.licenseOwner.document}/${license.licenseClass}`
				);
				const data = await res.json();
				...
				toaster.danger('ERROR AL CALCULAR COSTO Y VIGENCIA DE LA LICENCIA.', {
					duration: 5,
				});
			}
		} else {
	...
	const handleSubmit = async (e) => {
		...
		if (costIsCalculated) {
			try {
				...
				const res = await fetch(`http://localhost:9090/license`, config);
				switch (res.status) {
					case 200: {
					...
					case 403: {
						toaster.danger('EL TITULAR NO ES APTO PARA ESTA LICENCIA.', {
							duration: 5,
						});
						break;
					}
				...
				toaster.danger('No se pudo guardar la licencia. Reintententelo.', {
					duration: 5,
				});
				// Ver si pongo algo aca
			}
		} else {
			toaster.warning('Falta calcular costo y vigencia de la licencia', {
				duration: 5,
			});
		}
	};
